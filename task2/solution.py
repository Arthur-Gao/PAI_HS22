...
class MNISTNet(nn.Module):
    def __init__(self,
        ...
        self.layer2 = nn.Linear(100, 512)
        self.layer3 = nn.Linear(512, 1024)
        self.layer4 = nn.Linear(1024, 512)
        self.layer5 = nn.Linear(512, 100)
    ...
    def train(self):
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                current_logits = self.network(batch_x)
                loss = F.kl_div(current_logits[2], current_logits[1], log_target=True, reduction = 'batchmean') + F.nll_loss(F.log_softmax(current_logits[0], dim=1), batch_y, reduction='sum')
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        :param in_features: Number of input featuresBayesianLayerBayesianLayer
        ...
        self.prior = ScaleMixtureGaussian(torch.tensor(0.5), torch.tensor(np.exp(-0)), torch.tensor(np.exp(-6)))
        ...
        self.weights_var_posterior = MultivariateDiagonalGaussian(
            mu = nn.Parameter(torch.zeros((out_features, in_features))),
            rho = nn.Parameter(torch.ones((out_features, in_features)) * -2)
        )
        ...
        if self.use_bias:
            ...
            nn.Parameter(torch.zeros((out_features)), requires_grad=True),
            nn.Parameter(torch.ones((out_features)) * -4.5, requires_grad=True))        
...
class BayesNet(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: typing.Tuple[int, ...], out_features: int):
        ...
        print("number of layers ", num_affine_maps)
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        log_prior = torch.empty(0)
        log_variational_posterior = torch.empty(0)

        cur_features = x
        for idx, current_layer in enumerate(self.layers):
            features = current_layer(cur_features)
            if idx < len(self.layers) - 1:
                features = self.activation(features[0])
                log_prior = torch.cat([log_prior, features[1]])
                log_variational_posterior = torch.cat([log_variational_posterior, features[2]])
            cur_features = features
            
        output_features = cur_features[0]
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        d = self.mu.numel()
        log_lik = -d/2 * torch.log(2*np.pi*self.sigma**2) - 1/(2*self.sigma**2) * torch.sum(torch.norm(values - self.mu)**2)
        return log_lik

    def sample(self) -> torch.Tensor:
        # TODO: Backprop_4. You need to complete the sample function 
        # for the Univariate Gaussian distribution. 
        sample = self.standard_normal.sample(self.mu.size()) * self.sigma + self.mu
        return sample
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        d = self.mu.numel()
        log_lik = -.5 * (2 * torch.sum(torch.log(self.sigma)) + torch.sum(torch.norm((values - self.mu)/self.sigma)**2) + d * torch.log(torch.tensor(2 * np.pi)))
        return log_lik
...
class ScaleMixtureGaussian(ParameterDistribution):
    def __init__(self, pi: torch.Tensor, sigma1: torch.Tensor, sigma2: torch.Tensor):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        d = values.numel()
        log_lik = -.5 * (2 * torch.sum(torch.log(self.sigma1)) + 2 * torch.sum(torch.log(self.sigma2)) + torch.sum(torch.norm((values)/self.sigma1)**2) + torch.sum(torch.norm((values)/self.sigma2)**2) + 2 * d * torch.log(torch.tensor(2 * np.pi)) - 2 * (torch.log(self.pi) + torch.log(1 - self.pi)))
        return log_lik

    def sample(self) -> torch.Tensor:
        sample = self.pi * self.gaussian1.sample(self.sigma1.size()) + (1 - self.pi) * self.gaussian2.sample(self.sigma2.size())
        return sample
